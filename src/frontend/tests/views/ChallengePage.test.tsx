...
describe("ChallengePage", () => {
  ...
  it("should show loading state while checking camera permissions", () => {
    ...
    // Assert
    expect(
      screen.getByText("Checking camera permissions..."),
    ).toBeInTheDocument();
  });
  ...
  it("should show webcam access denied message when camera permission is denied", async () => {
    ...
    expect(
      screen.getByText(/PoPAI requires webcam access/),
    ).toBeInTheDocument();
  });
  ...
  it("should render the challenge interface when webcam access is granted", async () => {
    ...
    expect(
      screen.getByText("Webcam feed will appear here"),
    ).toBeInTheDocument();
  });
  ...
  it("should start a random challenge when Start Challenge button is clicked", async () => {
    ...
    vi.mocked(backendService.startVerificationChallenge).mockResolvedValue(
      mockChallenge,
    );
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Challenge: Blink twice slowly"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("should handle microphone challenges correctly when mic access is granted", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText('Challenge: Say "PoPAI is cool"'),
      ).toBeInTheDocument();
    });
